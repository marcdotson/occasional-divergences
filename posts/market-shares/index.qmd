...
1. Calculate posterior predictions by taking the dot product of the parameter estimates and *every* possible combination of the attribute levels. Do this for *every* (post warm-up) draw from the posterior.
2. Take the inverse logit of these posterior predictions to convert them to the probability space.
3. Average these posterior predicted probabilities across draws for a given attributes' levels.
...
- Issue with how the outside option is coded (all zeros) and it inflating the marginal means? From Andrew:

> My theory is that the zero option is somehow messing with the manual matrix multiplication and lumping zeros together in strange ways that lead to a tripling of the estimates.
