...
- Move project details over and then `uv run` to get everything setup?! USE GITHUB??????

- Instance is disconnecting/timing out after cloning repo and trying to run `uv run`. Is this just a problem with using a t2.micro? With using Positron?
- Try using a different instance type, like t2.medium, which has more memory and CPU power.

Doesn't recognize uv-installed Python version...
