...
date: "2024-09-31"
description: |
  Spinning up and using virtual machines to break through computational bottlenecks can be challenging. This post is designed as a gentle introduction, though your mileage may vary.
categories:
  - r
  - python
  - stan
...
It's not uncommon to run into computational bottlenecks when running models, Bayesian or otherwise. I've often had access to a dedicated computer or server for such tasks or, in classic academic fashion, I've been comfortable with letting models run for weeks. But these options aren't always available and they might not actually solve the problem. Cloud computing is clearly the solution. However, breaking into using cloud computing can make you feel the need to (with apologies) adapt the adage often applied to needing regular expressions: "If you have a problem that requires cloud computing, congratulations, you now have two problems.""

There are a number of cloud service providers, AWS arguably chief among them. However, AWS can do *everything* when what I really need is the ability to quickly spin up and temporarily use a virtual machine that can easily run R and/or Python (and often Stan). One of the primary reason VS Code has been so hyped is the use of its SSH extension for easily tunneling into and using such a virtual machine. However, the virtual machine needs to be spun up in the first place. This definitely feels like a *draw the rest of the owl* situation.
...
A virtual machine (VM) is a machine (i.e., computer) that is actually some subset of one of many servers in a data center somewhere (i.e., the cloud). In other words, it's not *actually* another computer, but rented time on a server, hence it being a "virtual" machine. But before we can set (or spin) up a VM, we need to prepare the environment we want to run on the VM. Also referred to as a machine image, this is a [reproducible environment](https://occasionaldivergences.com/posts/rep-env/) writ large: the operating system, applications, programming languages, and packages.
