...
Whenever you add new packages, or update the version of packages you're using, call `renv::snapshot()` to update the lockfile and `requirements.txt` accordingly. When someone else is trying to reproduce your environment, once they have your project working directory, they simply need to call `renv::restore()` to install the correct version of the required packages. (Note that as of writing this, `renv::restore()` will only install the R packages. You'll need to install the Python packages manually by calling `!pip install -r requirements.txt`.)

::: {.callout-note title="Python Virtual Environments (Part Two)"}
If we were just working with Python, after navigating to our project's working directory in the command line, we would create a virtual environment for our project by running `python -m venv venv`. This would create a `/venv` folder in the working directory that will contain the project library. This `/venv` folder is the Python-only workflow equivalent to the `/renv` folder that {renv} created for us in the R and Python workflow.

We would also need to activate this virtual environment whenenver we worked with the project by running `source venv/bin/activate`. To update our lockfile equivalent, we would run `pip freeze > requirements.txt` (which is actually what `renv::snapshot()` runs for us). And to install the packages on a new machine, we would run `pip install -r requirements.txt` (which is actually what `renv::restore()` *should* run for us).
:::
