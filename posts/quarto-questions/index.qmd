...
The code block is fenced with the language in braces, but instead of attributes in the braces, or the old R Markdown code chunk options, Quarto code blocks use the new hashpipe operator `#|` where the options follow YAML notation. This allows for two things. First, the same YAML options can be used for languages other than R. Second, if there are code cell options that you want to apply to the entire document, you can just move them into the header:
...
One finicky thing to note: YAML keys and values all use lowercase values. If you're switching from R Markdown, don't get your `TRUE` and `true` mixed up.
