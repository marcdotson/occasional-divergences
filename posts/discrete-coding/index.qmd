...
- Read and reference previous post(s) on choice modeling.
- Compare this `hmnl_noncentered` to what is used for why-donors-donate.
...
Let's generate data with discrete explanatory variables from a hierarchical multinomial logit.

```{stan generate-hmnl-data, output.var="generate_hmnl_data", eval=FALSE}

```

Call `generate_hmnl_data` from R.

```{r call-generate-hmnl-data, eval=TRUE}
#| message: false
