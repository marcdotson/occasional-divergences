...
Libraries and modules (a kind of sub-library) are imported with commonly accepted aliases in order to shorten the namespace reference. For example, the typical Polars alias is `pl`.
...
There's no way around this---visualizing data is where {ggplot2} simply reigns supreme, so much so that Posit has been investing in [plotnine](https://plotnine.org), a {ggplot2} port for Python. Maybe Posit will eventually facilitate a kind of polyglot grammar of graphics, however in the spirit of this post, let's consider a genuinely "Pythonic" tool.

If NumPy is base R, [matplotlib](https://matplotlib.org) is plotting in base R. If that analogy holds, matplotlib is an acquired taste, so much so that [seaborn](https://seaborn.pydata.org) was developed to supplement matplotlib, much like pandas was developed to supplement NumPy. While we don't have a Polars-like replacement (come on Hadley, a polygplot {ggvis} is written in the stars!) we do have seaborn.objects, a module deliberately built with the consistency of a grammar of graphics in mind that also attempts to eliminate the need to drop down to matplotlib for fine-tuning. Let's illustrate some common visualizations using seaborn.objects and {ggplot2} and gain some intuition for the differences.
...
Starting with a familiar-looking `so.Plot()` function which instantiates a Plot object. With a consistency that isn't present with {ggplot2} (I'm looking at you `|>` vs. `+`), there are a set of methods applicable to a Plot object, starting with `.add()`.
...
Once again, we see the use of an alias to shorten the namespace reference. Here, the typical alias for the seaborn.objects module is `so`. We also see one of the limitations of method chaining (and thus object-oriented programming): Methods are specific to libraries meaning we can't pass data wrangled in one library to be visualized in another like we can using the function-like pipe in R.
