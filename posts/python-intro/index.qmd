...
```{python}
test = pl.read_csv(os.path.join('posts', 'python-intro', 'data', 'test.csv'))

test

test.drop_nulls(pl.col('star_rating')).select(pl.col(['review_time', 'star_rating']))

(test
  .drop_nulls(pl.col('star_rating'))
  .select(pl.col(['review_time', 'star_rating']))
  .with_column(pl.col('review_time')
    .str.strptime(pl.Datetime, fmt="%m/%d/%Y")
  )
)
```
