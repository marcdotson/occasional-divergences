...
Once again, we see the use of an alias to shorten the namespace reference. Here, the alias convention for the seaborn.objects module is `so`.

We also see one of the limitations of method chaining (and thus object-oriented programming): Methods are specific to libraries, meaning we can't pass data wrangled in one library to be visualized in another like we can using the functional pipe in R.
